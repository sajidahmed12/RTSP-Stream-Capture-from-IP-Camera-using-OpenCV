...
arg_parser.add_argument('--ip', action='store', type=str, required=False, default='192.168.100.3', help="ip_camera's ip address")
...
arg_parser.add_argument('--save-video', action='store', type=bool, required=False, default=False , help="Save the RTSP video in recordings directory")
...
#example: rtsp://admin:altrsns007@192.168.3.168/Streaming/tracks/3201/?starttime=20230501T154200Z&endtime=20230501T174602Z

if args.live:
    RTSP_URL = 'rtsp://'+str(args.cam_user)+':'+str(args.password)+'@'+str(args.ip)+'/Streaming/channels/'+str(args.num)
    print(RTSP_URL)
if not args.live:    
    RTSP_URL = 'rtsp://'+str(args.cam_user)+':'+str(args.password)+'@'+str(args.ip)+'/Streaming/tracks/'+str(args.num)+'?starttime='+args.start_time+'Z'
    print(RTSP_URL)
...
video_filename = datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")+"_cam_"+str(args.num)
